...
{ t:'The first commit',
  ...
  story:`<p>A is the root — the only commit with no arrow. The tag floating above it is <mark>main: not a container, just a name pointing at one commit</mark>. HEAD marks where you're standing.</p>`,
...
{ t:'Committing moves your branch',
  lede:'B lands, and main slides forward to it — automatically.',
  story:`<p>That's the deal with an attached HEAD: every commit you make, <mark>the branch you're on moves with you</mark>. Nobody ever moves main by hand; committing is what moves it.</p>`,
...
{ t:'A branch costs nothing',
  ...
  story:`<p>Both tags point at B; git wrote a 41-byte file and that's the whole branch. HEAD hops onto feature, so <mark>your next commit will move feature, not main</mark>.</p>`,
...
{ t:'Work lands where HEAD rides',
  lede:'F1 grows from B; feature follows it. main hasn’t budged.',
...
{ t:'Switching is teleporting',
  lede:'HEAD hops home to main; your files become B’s snapshot again.',
  story:`<p>Nothing about the graph changed — switching only moves HEAD and rewrites your working files to match. <mark>Branches are parallel worlds you can hop between</mark>, cheap enough to make one per idea.</p>`,
  cmd:'$ git switch main',
  plumbing:null,
  present:['A','B','F1'], dim:[], ghost:[], halo:[], notes:{},
...
{ t:'You know the grammar now',
  lede:'Blocks never move. Tags do. Everything else on this site is built from that.',
...
commitNote(id,st,i){
  if(id==='A')return `<p>The root commit — no parent arrow. Every other commit in every animation ultimately points back here.</p>`;
  return '';
},
refCards:{
