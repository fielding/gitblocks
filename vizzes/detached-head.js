...
{ t:'Look around',
  ...
  story:`<p>Build it, run the tests, bisect a bug, read old code. Visiting is the point, and it's safe: <mark>a checkout never changes history</mark>, only your working tree.</p>`,
  cmd:null, plumbing:null,
  present:['A','B','M1','M2'], dim:[], ghost:[], halo:[], notes:{B:'you are here'},
  refs:{main:'M2', head:{at:'B'}} },
...
{ t:'Committing while detached',
  ...
  story:`<p>E1's parent is B, and HEAD rides along as always. But look at the tags: <mark>no branch points at E1</mark>. It's reachable only through HEAD itself.</p>`,
...
{ t:'Back to the present',
  lede:'HEAD goes home. experiment keeps the idea.',
  story:`<p>The whole loop: detach to visit, commit if something comes of it, <mark>name it if it's worth keeping</mark>, then switch back.</p>`,
  cmd:'$ git switch main',
  plumbing:null,
  present:['A','B','M1','M2','E1'], dim:[], ghost:[], halo:[], notes:{},
...
commitNote(id,st,i){
  ...
  if(id==='E1')
    return `<p>Safe now, because the experiment branch owns it. Made from a detached HEAD, kept by an ordinary pointer.</p>`;
  if(id==='B'&&i<=2)
